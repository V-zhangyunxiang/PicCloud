...
当应用调用 `startActivity` 时，通过 `Instrumentation` 跨进程请求系统的 **ATMS**，ATMS 接收到请求后，会解析 Intent、检查目标 Activity 是否在 Manifest 中注册、检查权限，根据启动模式确定目标 Activity 所在栈，若目标进程不存在则由 **Zygote** fork 新进程并执行 `ActivityThread.main`，ActivityThread 通过 `ApplicationThread` 向 ATMS 报到，ATMS 回调应用进程，应用进程通过 **Handler H** 切换到主线程，利用 `Instrumentation` **反射**创建 Activity 实例并触发从 `onCreate` 到 `onResume` 的生命周期。
...
- **本地广播**：仅在应用内部传递，其他应用无法接收，安全且高效。
    
- **全局广播**：可跨应用传递，所有监听该广播的应用都能接收（可设置权限或包名限制）。
...
mAttachedScrap 存放已被 detach 下来、暂时离开屏幕但尚未被回收或复用的 ViewHolder，正常滑动过程触发。

mChangedScrap 存放数据已经变更但还未重绘的 ViewHolder，数据变更、动画场景触发，用于缓存那些需要重新绑定的 ViewHolder。
...
mCachedViews 是 RecyclerView.Recycler 内部用于缓存**完全滑出屏幕的 ViewHolder**的 ArrayList。它会根据 ViewType 匹配和缓存顺序，优先复用刚滑出屏幕的 ViewHolder，提升滑动回滚的性能。默认缓存大小为 2，超出后最旧的 ViewHolder 会被移入全局的 RecycledViewPool。复用时会重新绑定数据以适应新位置。
...
## RecyclerView 回收 ViewHolder 的过程

1. 优先放入 mCachedViews（本地缓存，未满时）。
2. mCachedViews 满了，就把最旧的移到 RecycledViewPool（全局池）。
3. 不能缓存的，直接丢到 RecycledViewPool 或销毁。

## RecyclerView 复用 ViewHolder 的过程

1. 先从 mAttachedScrap 查找可用的 ViewHolder。
2. 没有就从 mCachedViews 查找。
3. 再没有就从 RecycledViewPool 查找。
4. 都没有的话，才新建一个 ViewHolder。
...
图片在内存中占用的空间与它在磁盘上的文件大小(压缩包大小)**没有直接关系**，而是由图片**分辨率**和**像素格式**决定的，图片在磁盘上通过 JPEG 或 PNG 算法进行了压缩，但显示时必须解压成位图。 在实际开发中（如 Android/iOS），如果显示区域比原图小，我们会进行“采样缩放”（Downsampling），只加载缩放后的像素到内存中，从而节省空间。内存占用的计算公式很简单：`宽 × 高 × 每像素字节数`。
...
Glide 结合了**倍数缩放**(1)和**精准缩放(2)**两种方式:
...
2. **`CustomTarget`** 适用于**非 View 场景**（比如后台下载图片处理）。使用它时**必须手动指定尺寸**，**并且一定要在 `onLoadCleared` 中释放对 Bitmap 的引用**，否则会导致 Bitmap 复用冲突显示错乱或程序崩溃。
...
一种更高级、更彻底的架构模式，通常特指**业务维度**的拆分。每个业务组件（如 `home`, `user`, `video`）都是一个可以**独立开发、编译、测试、运行**的完整“小应用”（即 `com.android.application`）。通过**路由框架**进行通信，最终由 **“主App”壳工程** 组装成一个完整的应用。它解决了**业务耦合**和**团队并行开发**的问题。
...
- **Mono-Repo**
 ...
 - **Multi-Repo**
   每个独立的模块都拥有自己单独的代码仓库，通过远程二进制依赖各仓库组件，每个模块有自己独立的版本历史和发布节奏。
   
建议采用“**主干 Mono-Repo + 外部依赖**”的模式。
 - 将 **稳定、变更不频繁、被多个项目使用** 的底层库（如加密库、公司通用工具SDK）放到独立的 Multi-Repo 中，以二进制形式依赖。    
...
- **路由框架** 
  ...
  使用路由框架（如 ARouter 等）来管理组件间的跳转和参数传递。通过 URL 或路径名导航到其他组件，并携带必要的参数。
...
*   **核心优势**：
    ...
    3.  **官方唯一主推**：**Support 库在版本 28.0.0（对应 Android 9.0）之后已经彻底停止更新**，所有的新功能、新组件（如 Compose、Room、ViewModel 等）全部只在 AndroidX 中提供 [4]。
...
在项目的根目录下找到 `gradle.properties` 文件，添加以下两行极其关键的配置 [5]：
```properties
# 强制要求项目使用 AndroidX 相关的包，而非 Support 包
android.useAndroidX=true

# 开启 Jetifier 工具。它的作用是：如果你引入的第三方旧 SDK 还在使用 Support 库，
# 编译时它会自动将这些旧 SDK 里的 Support 引用动态替换为 AndroidX 的引用。
...
4.手动排查与修复冲突 (收尾工作)
自动化工具虽然强大，但往往无法处理所有边缘情况，你需要手动进行以下排查：
*   **类名与包名冲突**：检查代码中是否有遗漏的旧包名。例如，最常见的冲突是 `android.support.v4.app.Fragment` 没有被成功替换为 `androidx.fragment.app.Fragment`，导致编译报错 [8]。遇到此类报错，手动修改 `import` 即可（类名、方法名和字段名本身在迁移前后是没有变化的 [5]）。
*   **更新第三方库**：虽然 `enableJetifier` 能临时解决第三方旧库的兼容问题，但它会显著拖慢编译速度。最佳实践是：去 Github 或官方文档检查你使用的所有第三方库，将它们统统升级到支持 AndroidX 的最新版本。当所有第三方库都原生支持 AndroidX 后，你就可以在 `gradle.properties` 中将 `android.enableJetifier=false` 关闭，从而大幅提升编译速度。
...
​      -> onInterceptTouchEvent() 返回 false 表示不拦截-> ViewGroup 遍历所有的子 View 找到被触摸的 View，调用该 View 的 dispatchTouchEvent
