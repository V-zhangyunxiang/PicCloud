...
- **Android Gradle 插件 (AGP)**：建议升级至 **8.5.1 或更高版本**。
- **NDK 版本**：建议升级至 **r27 或更高版本**。新版 NDK 默认支持 16KB 对齐，能大幅简化配置工作 116。
...
在 Android 开发中，理解这些分类是为了解决内存泄漏和 OOM 问题。

**常用工具：**
1.  **Android Studio Profiler**：可视化查看 Java Heap 和 Native Heap 的实时曲线，支持抓取 Heap Dump 分析对象引用链。
2.  **命令行工具**：
    ```bash
...
** MEMINFO in pid 12345 [com.example.app] **
                ...
                ------       ------   ------   ------   ------   ------   ------
  Native Heap:   45000        44500        0      100    80000    40000    40000
  Java Heap:     15000        14800        0        0    20000    12000     8000
  Code:          10000         2000     8000        0
  Stack:            50           50        0        0
  Graphics:      30000        30000        0        0
  System:         5000         5000        0        0
  TOTAL:        105050       ......
```

*   **Pss Total (Proportional Set Size)**：应用实际占用的物理内存（包含按比例分摊的共享库内存）。**这是系统判断是否杀掉进程的依据。**
*   **Private Dirty**：应用独占且已修改的内存（无法被换页出去）。这是你优化内存时最应该关注的指标，因为它完全由你的 App 产生。
*   **Graphics**：如果这个数值很高且持续增长，说明有图片资源未释放或 OpenGL 纹理泄漏。
