...
singleTop：栈顶复用模式。若 Activity 已在栈顶则会复用并回调 onNewIntent，如果不在会创建一个新的实例，**常用于通知栏跳转**。
...
singleInstance：单实例模式。该 Activity 独占一个任务栈，且该栈内**永远只有这一个实例**。常用于**通话**、**闹钟**等系统级应用，这些功能在整个系统中应该是全局唯一的。

在 onNewIntent 方法中，建议先手动调用 setIntent(intent) 更新 Activity 的 Intent，确保之后调用 getIntent 能够获取到最新的 Intent 数据。
...
2. **软件包可见性**：在 Android 11 及以上版本，必须在 `<queries>` 标签中声明**目标包名或Action**，否则无法检测到目标 App。
3. **稳定性保护**：跨 App 调用极易因对方未安装或版本更迭导致崩溃，必须配合 `resolveActivity` 检查或 `try-catch ActivityNotFoundException`。

## AppCompatActivity、FragmentActivity、ComponentActivity 区别和最新的变化

| 特性       | `ComponentActivity`                             | `FragmentActivity`                      | `AppCompatActivity`                                |
| -------- | ----------------------------------------------- | --------------------------------------- | -------------------------------------------------- |
| **核心定位** | Jetpack 架构组件的基石                                 | 支持 `Fragment` 的 Activity                | 提供 Material Design 和向下兼容特性的 Activity               |
| **主要功能** | 提供 `Lifecycle`、`ViewModel`、`SavedState` 等现代架构支持 | 包含 `FragmentManager`，管理 `Fragment` 生命周期 | 支持 `ActionBar`、`AppCompat` 主题、`Material Design` 组件 |
| **继承关系** | 直接继承自 `Activity`                                | 继承自 `ComponentActivity`                 | 继承自 `FragmentActivity`                             |
| **适用场景** | **Jetpack Compose 项目** 的首选                      | 需要**使用 `Fragment`** 的 View 系统项目         | **绝大多数传统 View 系统项目**的默认选择                          |

### 该如何选择？

- **选 `ComponentActivity`，如果...**
    
    - 你的项目是一个**纯 Jetpack Compose** 项目。它是为 Compose 设计的轻量级基础，没有任何传统 View 系统的负担。
        
- **选 `AppCompatActivity`，如果...**
    
    - 你的项目是基于**传统 XML 布局的 View 系统**。这是绝大多数现有项目和传统开发模式的标准选择，它能保证你的应用在不同 Android 版本上拥有一致的 Material Design 外观和体验。
        
- **`FragmentActivity` 还用吗？**
    
    - **通常不需要直接使用**。在绝大多数情况下，`AppCompatActivity` 已经继承自 `FragmentActivity`，功能上完全覆盖。除非你有极其特殊的理由，否则直接用 `AppCompatActivity` 或 `ComponentActivity` 即可。

## Android 版本迭代对于 Activity 的更新和变化有哪些？

**1. 生命周期优化（Android 12 核心变化）**

> - **根 Activity 返回不再销毁**：点击返回键从桌面返回时，根Activity（Launcher Activity）不再执行`onDestroy()`，只执行到`onStop()`。
>     
> - **效果**：再次打开变为“热启动”，速度更快；`ViewModel`中的数据得以保留。
>     
> - **开发要求**：不要重写`onBackPressed()`并手动调用`finish()`，应使用`OnBackPressedCallback`。

**2. 返回导航重构（Android 13+ 重大变更）**

> - **`onBackPressed()`废弃**：Android 13 起，返回事件处理转为“预先处理”模型。
>     
> - **新方案**：使用`OnBackPressedCallback`(AndroidX)处理返回逻辑。
>     
> - **原因**：更好地支持手势导航，提供更一致的返回体验。

**3. 启动管控与适配（Android 10 ~ 14）**

> - **后台启动限制（Android 10）**：应用在后台不能随意启动 Activity，必须由用户交互直接触发。
>     
> - **架构拆分（Android 10）**：`ActivityTaskManagerService`(ATMS) 从`ActivityManagerService`中拆分出来，独立管理 Activity 和 Task。
>
> - **强制声明`android:exported`（Android 12）**：任何带`<intent-filter>`的 Activity 必须显式声明`exported`属性，否则无法安装。
> 
> - **隐式 Intent 限制（Android 14）**：隐式 Intent 只能启动已导出的组件，启动未导出的 Activity 必须使用显式 Intent。
...
## Android 版本迭代对于 Service 的更新和变化有哪些？

**为了应对越来越严苛的系统限制，Android 官方推荐的后台任务解决方案，已经从直接使用 `Service`，演进为以 `WorkManager` 为主、`ForegroundService` 为辅的组合模式。** 而 `ServiceCompat`只是为了在过渡期保持兼容性的工具。

### 核心概念对比

| 概念                      | 核心定位                                         | 关键特点                                           | 当前状态 (2026年)                      |
| ----------------------- | -------------------------------------------- | ---------------------------------------------- | --------------------------------- |
| **`Service`**           | Android四大组件之一，用于执行后台任务的基础组件。                 | 分为**后台服务**和**前台服务**。普通后台服务在后台运行极易被系统限制。        | **不推荐**直接使用普通后台服务。                |
| **`ForegroundService`** | 一种特殊的前台服务，用于执行用户可感知的、高优先级的任务[。               | 必须显示一个持续运行的通知，系统优先级高，不易被杀死。                    | **保留使用**，但仅限特定场景，且使用条件日益严格。       |
| **`WorkManager`**       | Android Jetpack 组件，用于处理**可延期**的后台任务。         | 自动选择最佳执行方式(JobScheduler 等)，保证任务执行，设备重启后任务仍可恢复。 | **官方首选**的通用后台任务解决方案。              |
| **`ServiceCompat`**     | AndroidX 库中的辅助类，用于在旧版 API上兼容使用新版本Service 特性。 | 提供如`startForeground()`等方法的向后兼容版本。              | **过渡工具**，在新项目中直接使用AndroidX 核心库即可。 |
### 关系演变：为何用 WorkManager 取代普通 Service？

随着 Android 系统对省电和隐私的重视，普通后台`Service`的使用受到了越来越多的限制。这个演变过程大致如下：

1. **Android 8.0 (API 26) 的分水岭**：系统规定，应用进入后台后，其普通后台`Service`会在几分钟内被强制停止。此后，后台任务要么转为**前台服务**（`ForegroundService`），要么使用`JobScheduler`等系统调度器。
    
2. **Android 12 (API 31) 的收紧**：系统进一步限制了应用在**后台**启动`ForegroundService`的能力。
    
3. **Android 14 (API 34) 的规范化**：要求所有`ForegroundService`必须在`AndroidManifest.xml`中**显式声明其服务类型**（如`mediaPlayback`、`location`等），并申请对应的权限。

**结论**：在严格的限制下，直接启动一个普通的后台`Service`已基本不可行。而`WorkManager`作为一个高层次的封装，能自动处理这些复杂的版本兼容问题，因此成为首选。

### 最新的推荐用法

#### 1. 大多数情况：使用 `WorkManager`

对于绝大多数不需要用户立即感知的后台工作(如数据同步、日志上传、图片处理等)，`WorkManager`是最佳选择。

- **优势**：它会根据系统情况(如网络、电量)智能调度，保证任务一定会被执行，并且兼容从Android 4.0 (API 14) 开始的所有版本。
#### 2. 特定场景：使用 `ForegroundService`

只有当任务对用户**高度可见且必须立即执行**时，才应使用前台服务。常见场景包括：

- 音乐/视频播放 (`mediaPlayback`)
    
- 导航或位置追踪 (`location`)
    
- 语音/视频通话 (`phoneCall`)
    
- 屏幕录制或投影 (`mediaProjection`)

### 如何正确使用 `ForegroundService`

1.**声明权限和类型**

在 `AndroidManifest.xml` 中，除了声明服务，**必须**加上 `android:foregroundServiceType` 属性。同时，需要根据你的服务类型申请对应的权限。

```xml
<!-- 1. 申请前台服务权限 -->
<uses-permission android:name="android.permission.FOREGROUND_SERVICE" />
<!-- 2. 若你的类型是 location，需额外申请；如果是 mediaPlayback 则申请对应权限 -->
<uses-permission android:name="android.permission.FOREGROUND_SERVICE_LOCATION" />
<!-- 3. 如果通知需要显示（Android 13+），还需申请通知权限 -->
<uses-permission android:name="android.permission.POST_NOTIFICATIONS" />

<application ...>
    <!-- 4. 声明 Service，并显式指定类型（Android 14 强制要求），必须和实际做的事匹配，如果不匹配，               系统会抛出 ForegroundServiceTypeNotAllowedException。
     -->
    <service
        android:name=".MyForegroundService"
        android:foregroundServiceType="location"   <!-- 根据实际场景填写 -->
...
写法和普通 `Service` 完全一样，只是必须在 `onCreate` 或 `onStartCommand` 中调用 `startForeground()`。
...
class MyForegroundService : Service() {
    ...
    override fun onCreate() {
        super.onCreate()
        // 1. 创建通知（必须，因为前台服务强制显示通知）
        createNotification()
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        // 执行你的后台任务（如上传文件、播放音乐、记录位置等）
        return START_STICKY // 或 START_NOT_STICKY，根据需要
    }

    private fun createNotification() {
        // 2. 对于 Android 8.0+，必须创建通知渠道
        ...
        // 3. 构建通知（Android 13+ 需用户授予 POST_NOTIFICATIONS 权限）
        // 如果用户拒绝了 POST_NOTIFICATIONS 权限，你的通知将不显示，此时启动前台服务会失效。                    所以启动前最好先检查并申请该权限。 
        ...
        // 4. 核心：将 Service 提升为前台状态（传入唯一 ID 和通知）
        startForeground(1001, notification)
    }

    override fun onDestroy() {
        super.onDestroy()
        // 5. 退出时移除前台状态（可选，通常调用 stopForeground(true)）
        stopForeground(true)
    }
}
```
...
启动时**必须**使用 `startForegroundService()`，否则在 Android 8.0 以上会报错。服务启动后，必须在 **5 秒内**调用 `startForeground()`，否则系统会触发 ANR（应用无响应）。
...
1. 接收方通过 Binder 向系统服务 **AMS** 注册其 `IntentFilter`；

2. 当发送方调用 `sendBroadcast` 时，请求会跨进程传给 AMS。AMS 负责在全局注册表中进行**匹配**，并将匹配到的接收者放入 **BroadcastQueue** 队列中。

3. AMS 通过接口跨进程回调应用进程。应用进程在收到回调后，利用内部的 **Handler** 机制将逻辑切换到**主线程**，最终触发 `onReceive` 方法。

## Android 版本迭代对于 Broadcast 的更新和变化有哪些？

**1. 静态注册限制（8.0+）：**

仅允许系统核心广播静态声明，普通广播必须使用 `registerReceiver()` 动态注册。

**Android 14+** 动态注册时也必须强制指定`RECEIVER_EXPORTED` 或 `RECEIVER_NOT_EXPORTED`标记。

**2. 内部通信替代（废弃）：** 

LocalBroadcastManager 已废弃，因其存在生命周期泄漏风险且性能差，现完全由 **`LiveData` 或 `Flow`** 替代同进程通信。

**3.高危权限加锁：**

   **系统广播**携带的敏感数据，必须解锁对应权限才能接收。
   
- **位置**：接收位置更新的广播(ACTION_LOCATION_CHANGED)，前台需 `ACCESS_FINE/COARSE_LOCATION`，后台需额外 `ACCESS_BACKGROUND_LOCATION`(且 Android 11 后引导受限，需用户去设置页手动开)。
    
- **通知（13+）**：应用发送通知需 `POST_NOTIFICATIONS` 运行时权限；接收系统通知广播(`ACTION_NOTIFICATION_POSTED`) 则不需要该权限。
...
## Android 版本迭代对于 ContentProvider 的更新和变化有哪些？

**1. 工具补全阶段（7.0~8.0）**

> - **7.0 引入 FileProvider**：禁止`file://` URI跨应用传递，强制用`content://`，这是最常用的安全文件共享方案。
>     
> - **8.0 增强数据加载能力**：新增分页查询和`refresh()`刷新接口，支持大数据集分批加载。
>     

**2. 隐私管控阶段（10~11）**

> - **10 推行分区存储**：访问外部媒体文件必须通过`MediaStore`（本质上就是系统`ContentProvider`），不能直接用文件路径，`DATA`字段被废弃。
>     
> - **11 引入包可见性限制**：访问其他应用的 Provider，必须在`<queries>`中显式声明，否则无法找到。
>     

**3. 强制规范阶段（12~14）**

> - **12 强制声明`android:exported`**：只要有 intent-filter，必须显式写 true 或 false，否则无法安装。
>     
> - **14 收紧元数据访问**：调用`getType()`获取 MIME 类型，现在强制要求持有读权限。
>

## RecyclerView 每一级缓存具体作用是什么，分别在什么场景下会用到哪些缓存

RecyclerView 的缓存机制本质上是为了**减少数据绑定**和**减少反射创建 View**的开销。
...
1. **Fragment 视图的生命周期**（onCreateView 和 onDestroyView）
2. **Fragment 实例的生命周期**（onCreate 和 onDestroy）
...
getDefaultProguardFile('proguard-android-optimize.txt') : AGP 提供的默认混淆规则（位于 Android SDK 目录），包含通用优化规则（如保留 Android 系统类、四大组件），无需修改。

proguard-rules.pro：项目自定义混淆规则文件（位于模块根目录），用于保留业务类、第三方库类。
...
2. init 启动 **Zygote**，Zygote **预加载**系统资源，并通过 **Socket** 机制监听孵化请求。
3. Zygote 孵化 SystemServer，SystemServer 初始化 AMS 等多个系统服务。
4. 当 AMS 检测到系统就绪后，**它会通过 Zygote 孵化出第一个 App 进程-Launcher(有系统权限)**，展示桌面。
...
- **规避多线程死锁风险**。`SystemServer` 是一个**拥有几十个线程的复杂进程**，在多线程环境下执行 `fork` 极易导致子进程因无法获取父进程中未释放的锁而产生死锁。
...
- **核心目的**：
    1. **性能飞跃**：根据 Google 实测，16KB 页面可使应用启动速度平均提升 **3.16% - 30%**，系统整体响应效率提升 **5% - 10%**。
...
3. 当泄漏对象达到阈值，LeakCanary 会触发一次堆转储，计算出从 GC Roots 到泄漏对象的最短引用路径，生成一份详细的泄露报告。
...
- 只保留 `xxhdpi` 或 `xxxhdpi`目录的本地资源。
- 只保留 **`arm64-v8a`** abi 架构，动态下发 so。

## 事件分发机制

![View 树结构|429](https://github.com/V-zhangyunxiang/PicCloud/blob/master/data/View%20%E6%A0%91.png?raw=true) 
![image|660](https://github.com/V-zhangyunxiang/PicCloud/blob/master/data/%E4%BA%8B%E4%BB%B6%E5%88%86%E5%8F%91.png?raw=true) 
...
整个事件分发，只需分清楚**“DOWN 阶段”**和**“非 DOWN 阶段（MOVE/UP/CANCEL）”**，只需要搞清楚 **`ACTION_DOWN`** 时发生了什么，后续的 `MOVE` 和 `UP` 基本上是照着 DOWN 的结果执行的。

用一句话总结：

> **DOWN 负责"找到接收者"，MOVE 和 UP 负责"直奔那个确定的接收者"。**
...
### 一个 View B、View A，View A 50% 透明度是盖在 View B 的上面的蒙层，View B 上有一个 Button B，如何在点击 Button B 时有响应
...
*   在子线程更新在子线程创建的 View。
*   **`checkThread`**方法是在 onResume 之后执行的，所以如果在 onCreate 中开子线程更新 UI，如果执行速度够快，有可能不报错但不可靠。
...
因为子线程没有 Looper，主线程的 Looper 在 ActivityThread.main 方法中通过 prepareMainLooper 已经创建，子线程需要调用 Looper.prepare 和 Looper.loop 才能使用 Handler。
...
2.  **发送异步消息**  
    创建 Handler 时设置 `async` 为 `true`，该 Handler 发送的**所有消息自动标记为异步消息**(Asynchronous Message)。也可以通过 msg.setAsynchronous(true) 单独设置某一条消息为异步。
3. **屏障的作用**  
...
绘制流程可以分成 2 个阶段，DecorView 的创建和渲染。
...
`ViewRootImpl` 调用 `scheduleTraversals()`，通过 `Choreographer` 注册 VSync 回调，并**插入同步屏障确保绘制任务优先执行**。VSync 信号到来时，触发 `performTraversals()`，依次执行: 
...
在 onMeasure 默认的实现中，当 View 的测量模式是 AT_MOST 和 EXACTLY 时，View 的大小会被设置成**父容器**的 specSize，导致 wrap_content 看起来和 match_parent 效果相同。
...
**SurfaceView** 的核心价值是拥有**独立的 Surface 和渲染线程**，适合游戏、视频等高频绘制场景，它利用**双缓冲机制**避免画面撕裂。但它的 Surface 在 View 层级的下方，不支持动画变换。
...
*   **分代模型**（经典）：
    *   **年轻代 (Young Gen)**：
        *   **Eden 区**：新对象出生地。
        *   **Survivor 区 (From/To)**：存放 GC 后存活的对象（通常比例 8:1:1）。
...
*   **标记-清除**
    ...
    缺点：产生大量不连续的碎片，当需要较大空间时，需要多次触发回收。
...
> 1. 优先使用核心线程（常驻），因为它们是预先创建好的，响应最快。
2. 当核心线程忙不过来时，任务先入队列排队，而不是急于创建新线程，这样可以平抑突发流量，避免线程数量膨胀。
3. 只有当队列积压到极限时，才会创建非核心线程（临时工）来扩容处理。
4. 若最大线程数也满了，则执行拒绝策略。
...
> **比较的是：内存里的“当前实时值” 是否等于 我拿到的“旧值副本”。**

1. 线程 A 执行 `CAS(V, 100, 90)`，发现 V(100) == A(100)，**比较成功**，更新为90。
    
2. 线程 B 执行 `CAS(V, 100, 90)`，此时它去 **“比较”** 内存 V，发现 V 的当前值已经变成了 **90**，而自己手里的旧预期值 A 还是 **100**。
    
3. **比较失败（V != A）**！说明数据在你计算的过程中被别人动过了。
    
4. 线程 B 放弃这次更新，**要么重试（自旋）**，重新读最新的余额（90），再算新值（80），重新比较；**要么报错**。
...
**“局部”和“非局部”描述的是返回动作的影响范围**，Lambda 内**用 `return@label` 只退出自己**，是局部；**用 `return` 跳出 Lambda 并退出外层函数**，作用范围超出了局部，所以叫非局部。
...
- `Dispatchers.Default`: **CPU 密集型**任务。它使用一个与 CPU 核心数相关的线程池。例如：排序、复杂计算。
    
- `Dispatchers.IO`: **I/O 密集型**任务。例如：网络请求、读写文件、数据库操作。它使用一个共享的、更大的线程池。
    
- `Dispatchers.Main`: **在主线程（UI线程）上运行**。用于更新 UI。需要在 Android、Swing 等有 UI 主循环的平台中使用。
...
| **必须有初始值？**    | 否                  | **是**                                         | 否                                            |
| **重播**         | 每次收集都从头开始。         | 自动保留**最后 1 个**状态值，并立即提供给新收集者。                 | 可配置**重播缓存**(replay)，向新收集者发射最近 N 个值。          |
...
- **调用限制**：`suspend` 函数只能在协程或另一个 `suspend` 函数中调用，因为挂起和恢复依赖调用处提供的 `Continuation`（书签）对象。
- **非阻塞挂起**：`suspend` 关键字本身只是标记，真正触发挂起的是函数内部调用的挂起点（如 `delay`、`withContext`）。挂起时不会阻塞当前线程，线程可以继续执行其他任务。
- **编译实现**：编译器会对 `suspend` 函数做 CPS 变换，把它变成带 `Continuation` 参数的普通函数，并生成状态机代码，从而支持“暂停—恢复”的执行流程。
...
1. 短连接。一次请求-响应完成后，TCP 连接立即关闭，不会长期保留通道。

2. 无状态。HTTP 协议本身不存储任何客户端的上下文信息，服务器不会记住上一次请求的用户身份、操作记录，每一次请求都是完全独立的，服务器收到请求→处理→响应后，就彻底遗忘该客户端，若需要保持登录、会话状态，必须依赖 Cookie、Session、Token 等额外机制实现。

### HTTP 1.1

1.  新增了 Keep-Alive 复用 TCP 通道，但通信模型依然是「客户端请求→服务器响应」，服务器仍然无法主动推送。

2.  管道机制。同一个 TCP 连接里面，客户端可以同时发送多个请求。但**响应必须按发送顺序返回**，如果前面的请求处理缓慢，会产生队头堵塞问题。但由于队头阻塞问题严重，且服务器实现复杂，**现代浏览器和服务器实际上都禁用了管道机制**。并发策略是通过**浏览器多开 TCP 连接**来实现的。

### HTTP 2.0

1.  多路复用。允许在**单个 TCP 连接上并行**发送多个请求和响应，服务端不用按照顺序返回，避免了队头堵塞。

2. 使用二进制传输，比文本格式速度更快。

3.  头部压缩。压缩请求头信息，且相同的信息只发送一次。

4.  服务端推送。**可以“猜测”并主动将 CSS、JS 等资源推送给浏览器缓存**，但并不能随时主动推送。

### HTTP 3.0

**弃用了 TCP，转而使用基于 UDP 的 QUIC 协议**，极大提升了弱网环境下的速度和稳定性。也包含多路复用、二进制传输、头部压缩、服务端推送等特性。
...
1. 非对称加密(公钥/私钥)：**用于握手阶段协商密钥，公钥公开、私钥保密**，无法反向破解，性能较低。

2. 对称加密：用于数据传输阶段，加解密速度极快，适合大量数据传输。

3. 数字证书(CA签发)：防止公钥被篡改，验证服务器真实身份。

**流程如下:**

1.  客户端向服务器发起 HTTPS 请求，连接到服务器的 443 端口。
2.  服务器返回一个 CA 数字证书(包含公钥、域名、有效期)。
3.  客户端校验证书的合法性，验证通过后生成一个随机值，使用证书中的**公钥将这个随机值加密**后发送给服务端。
4.  服务器**使用私钥解密**这个随机值，然后使用这个随机值作为对称加密的密钥，用其将数据加密后传送给客户端。
5.  客户端使用这个随机值解密服务器传送来的信息，获取到明文的 HTTP 数据。

## HTTP 和 HTTPS 的区别

HTTPS 并不是一套独立的全新协议，它是在原有 HTTP 明文传输的基础上，在传输层与应用层之间增加 TLS/SSL 安全握手与加密通道，HTTP 的请求方式、语义、接口规范完全保留，只是数据传输链路被加密保护。

标准端口区分：HTTP 默认 80 端口，HTTPS 默认 443 端口。

![[Http VS Https.png]]

**适用场景**

  - HTTP：仅用于内网测试、局域网内部系统、非敏感静态资源、老旧内网设备交互，公网正式线上环境目前基本不再单独使用，主要受 Android 系统安全策略与行业规范限制。
  - HTTPS：所有公网正式上线的 App，尤其是涉及账号登录、密码、支付、个人信息等敏感数据的业务，目前行业标准为全站 HTTPS，是 Android 线上应用的唯一主流选择。

## 什么是 Socket 通信

Socket(套接字)不是网络协议，而是一套「编程接口」，是应用程序访问底层 TCP/UDP 传输协议的通用入口，两端设备通过 Socket 建立数据通道，完成字节流的收发，由 **IP 地址 + 端口号**唯一标识一台设备，是底层网络通信的基础，HTTP、WebSocket 等协议底层本质也都依赖 Socket 实现。

## Socket 有哪些分类

按照底层依托的传输层协议，分为两类，特性与用途差异极大。

**TCP Socket:** 基于 TCP 协议，面向**有连接、可靠传输**，通信前必须先完成**三次握手建立专属连接**，传输过程保证**数据有序、不丢失、不重复**，适用于对数据完整性要求高的场景(即时消息、文件传输、指令交互)。

**UDP Socket:** 基于 UDP 协议，面向**无连接、不可靠传输**，无需提前建立连接，直接向目标 IP + 端口发送数据包，开销小、延迟低，但不保证数据一定送达、顺序不乱，适用于对实时性要求高于可靠性的场景(直播弹幕、局域网设备发现、语音通话)。
...
> HTTP 是「一问一答、说完就断」，WebSocket 是「永久连线、双方互聊」。

## 在 Android 中如何自建长连接，有哪些常见的实现方式
...
### 核心实现步骤(以最通用的 WebSocket 为例)

无论选择哪种方式，实现一个稳定的长连接都需要完成以下核心步骤。以 Android 上最常用的 **OkHttp + WebSocket** 为例：

1.  **引入依赖**：在 `build.gradle` 中添加 OkHttp 依赖。
2.  **建立连接**：使用 `OkHttpClient` 的 `newWebSocket()` 方法创建连接，并传入 `WebSocketListener` 监听连接状态和消息。**所有回调在子线程，更新 UI 必须切换到主线程**。回调方法如下：
>    onOpen：连接成功
 >   onMessage：接收服务端消息
>    onFailure：连接失败/断开
3.  **实现心跳**：**这是保证连接存活的关键**。OkHttp 提供了 `pingInterval` 方法设置自动心跳间隔，这是最简单的保活方式。
4.  **实现重连**：在 `onClosed()` 或 `onFailure()` 回调中，使用 **指数退避算法(1s → 2s → 4s → 8s)** 等策略尝试重新建立连接。**WiFi / 移动网络切换时，主动断开旧连接并重建**。标记连接状态，防止重复重连。
5.  **管理生命周期**：在 Activity 或 Service 的 `onDestroy()` 等方法中主动关闭连接，避免资源泄漏。
6. **通信**：发送消息：调用 webSocket.send("内容")；断开连接：webSocket.close(1000, "退出")。
...
自建长连接在 Android 系统（尤其是国内定制系统）上，最大的挑战往往是**应用后台存活**和**省电策略**。你需要配合使用**前台服务 (Foreground Service)** 等方式，并针对不同厂商做适配，才能保证长连接的稳定性。

## Socket.IO 一种更高效的 WebSocket 实现

**[Socket.IO](https://socket.io/) 是一个基于事件驱动的实时双向通信库**，它在 WebSocket 的基础上提供了更高级、更易用的功能封装，**是 WebSocket 的一个“增强版”工具库**。它通过提供自动重连、事件机制、降级方案等特性，极大地降低了开发稳定实时通信功能的复杂度。

 **重要提示**
 
- **线程安全**：Socket.IO 的回调函数运行在**非 UI 线程**，更新界面必须通过 `runOnUiThread`、`Handler` 等方式切换到主线程。
    
- **网络安全**：从 Android 9 (API 28) 开始，默认禁止明文流量。如果服务器地址是 `http://` 而非 `https://`，需要在 `AndroidManifest.xml` 中进行配置以允许明文传输。
    
- **电量消耗**：Socket.IO 会与服务器保持长连接，这会消耗电量。官方文档明确指出，**它不适用于需要后台常驻的服务场景**。对于后台推送，应使用 FCM 等专用方案。

## 流媒体通信在 Android 里主要应用在哪些业务场景

流媒体的核心特征是**边下载、边解码、边播放**，无需等待完整文件加载完毕，专门针对音视频大数据流设计，在 Android 端主流业务场景如下:

**一、在线音视频点播**

短视频、长视频、影视剧、网课、有声读物、在线音乐等，是最普遍的场景，多使用 HLS 协议。

**二、实时直播业务**

娱乐直播、电商直播、游戏直播、户外直播，**对延迟有一定要求**，主流采用 RTMP、HLS。

**三、实时互动音视频**

直播间连麦、多人语音/视频通话、在线会议，**除流媒体播放外，还会搭配实时传输协议。**

**四、安防与物联网音视频**

家用智能摄像头、工地监控、车载摄像头、门禁可视对讲，需要**实时预览、视频回看**，是安防类App 核心能力。

**五、车载与大屏流媒体**

车机在线视频、车载电台、电视盒子投屏播放，属于嵌入式 Android 设备的典型场景。

## HLS、RTMP、RTP 三大主流流媒体协议

流媒体协议的延迟、稳定性完全由**底层传输协议 + 数据封装**方式决定。

**一、底层协议归属**

HLS：基于 HTTP(TCP)，苹果推出，Android 全兼容。

RTMP：基于 TCP，Adobe 私有协议。

RTP：基于 UDP(配合 RTCP 做质量控制)，专用实时传输协议。

**二、延迟数据（Android 端典型表现）**
...
**TCP 保证可靠性 + 切片传输(将视频切成 5-10 秒的 ts 文件)，客户端必须加载 1-2 个切片才能播放**，天然高延迟。

**3.2、RTMP 中低延迟原因**

**TCP 长连接 + 流式连续传输，无切片，实时推流实时播放**，仅受 TCP 重传机制影响。

**3.3、RTP 超低延迟原因**

**基于 UDP 无连接、无重传、无流量控制**，牺牲可靠性换极致实时性，适合视频通话、安防监控。
...
## Android 端做音视频流媒体播放，常用的系统/三方播放器组件有哪些

**一、系统自带播放器(轻量、无依赖)**

MediaPlayer：底层播放器，支持本地/网络音视频，支持 HLS/MP4，功能简陋。

VideoView：基于 MediaPlayer 封装的 UI 控件，直接布局使用，适合简单播放场景。

**二、企业级三方播放器(主流选型)**

**2.1、ExoPlayer**

Google 官方开源，支持 HLS/RTMP/DASH，自定义能力强，Android 开发首选。

**2.2、ijkPlayer**

基于 FFmpeg，全格式支持，兼容低版本 Android，适合安防、直播场景。

**2.3、GSYVideoPlayer**

基于 ExoPlayer/ijkPlayer 封装，UI 完善，支持手势、缓存、小窗口，开箱即用。

**2.4、VLC**

专业级播放器，支持极低延迟流媒体，适合监控场景。

## 流媒体播放出现卡顿、花屏、音画不同步，有哪些优化手段

- 协议切换：高延迟 HLS → 低延迟 RTMP/UDP。
- **自适应码率**：根据网络带宽切换清晰度(480P/720P)。
- **CDN 加速**：使用就近 CDN 节点，降低拉流延迟。
- **硬件解码**：开启 GPU 硬解，降低 CPU 占用。
- 缓冲优化：调整预缓冲大小，弱网增加缓冲时长。

90% 的问题通过 **硬解+自适应码率+CDN** 即可解决，专业场景使用 ijkPlayer/ExoPlayer 自定义配置。
## WebView 对外通信本质依赖什么网络协议

### 底层依赖协议

WebView 是 Android 内置的网页渲染引擎，对外加载资源、发起网络请求的底层完全依赖HTTP/HTTPS 协议，和手机浏览器原理一致。如果网页内部使用 WebSocket、流媒体等能力，也是网页自身基于标准网络协议实现，WebView 仅做载体转发，底层仍依托系统网络栈。
### 核心交互能力

**网络资源加载能力**

加载远程网页、HTML、CSS、JS、图片、视频等静态/动态资源，执行网页内部的接口请求、资源拉取，实现纯 H5 页面的网络访问。

**Java 与 JavaScript 双向跨语言通信**

区别于普通浏览器最核心的能力，也是 Android 混合开发(H5 + 原生)的基石。

1. 原生 Java 代码调用网页中的 JS 方法。
2. 网页 JS 代码调用 Android 原生 Java 方法。

借助该能力可以实现 H5 调用原生相机、定位、文件、蓝牙等系统能力，原生也可以向 H5 传递数据、控制网页逻辑。

## 什么是局域网 IP 直连通信，Android 的实现形式有哪些

**一、局域网 IP 直连通信定义**

局域网 IP 直连，指多台设备接入同一个路由器/WiFi 局域网，依靠内网 IP 地址寻址，不经过公网服务器中转，设备之间直接点对点通信。

核心特点：延迟极低、带宽高、不消耗公网流量、部署简单，主要用于设备联动、本地数据交互，广泛应用于智能家居、设备互联、文件传输场景。

**二、Android 主流实现形式**

**局域网 TCP/UDP Socket**

最基础、使用最广的方案，设备获取内网 IP 后，通过指定端口建立 Socket 连接，实现消息收发、大文件传输、指令控制，灵活性最高。

**UDP 广播/组播**

基于 UDP 实现的批量通信，主要用于**局域网设备发现**: 一台设备发送广播包，局域网内所有监听端口的设备都能接收，常用于设备上线探测、状态通知。

**WiFi Direct(WiFi P2P)**

又称 WiFi 点对点，无需依赖无线路由器，两台/多台 Android 设备可自行组建独立局域网，实现直连通信，典型场景：手机互传文件、无线投屏、多设备联机。
...
| 合成复用原则<br> | “多用组合，少用继承。”(继承是强耦合，组合是松耦合) | 坏例子：为了复用代码，让 `Dog` 继承 `Animal`，再继承 `LivingThings`...(继承链爆炸)。  <br>好例子：在 `Dog` 类里 **组合** 一个 `EatBehavior` 和 `RunBehavior` 对象，想怎么拼就怎么拼。 |
| ---------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
...
观察者模式的核心是“定义对象间的一对多依赖关系，**当被观察者(Subject)的状态发生变更时，所有依赖它的观察者(Observer)会自动收到通知并更新”**，其本质是通过“解耦观察者与被观察者”，让两者仅通过抽象接口交互，无需感知对方的具体实现。
