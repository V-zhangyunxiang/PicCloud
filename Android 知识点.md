...
Choreographer 监听 VSync 信号，SurfaceFlinger 在下一个 VSync 信号时收集所有 Surface 的内容进行合成，提交给显示硬件。
...
 Lambda 参数在编译后会生成匿名类对象，频繁调用会有内存开销。加上 `inline` 关键字后，编译器会将函数体和 Lambda 直接内联到调用处，消除匿名对象创建开销。
...
协程是一个线程框架，能够在一个代码块里进行多次线程切换，用同步的写法处理异步逻辑，告别回调地狱。

- 在 Activity 或 Fragment 中使用 lifecycleScope。

- 在 ViewModel 中使用 ViewModelScope。

- 不与任何特定的 Android 生命周期相关联的场景下，手动创建 CoroutineScope 来管理协程，需要你手动控制其生命周期，包括启动和取消协程(一般不推荐)。
...
**suspend 函数**只能在协程或者另一个 `suspend` 函数内部调用，**挂起的是协程，挂起函数在执行完成之后，协程会重新切回它原先的线程**。suspend 并没有实际挂起的作用，仅仅是函数的创建者对函数的使用者的提醒，强制耗时操作必须在子线程中进行。**非阻塞式指可以用看起来阻塞式的代码写非阻塞的操作。**
