...
**模块化**：代码/功能层面的拆分，模块间仍存在编译依赖，是「物理拆分、逻辑耦合」。
...
模块化是拆分，组件化是解耦：模块化解决「代码乱」，组件化解决「耦合深」，组件化是模块化的进阶形态。
...
每个模块**独立存放代码、资源**，命名空间前缀区分（如 user_  / pay_），避免资源冲突耦合。
...
应用启动时，路由框架通过**反射或直接调用**生成的路由表类，**将“地址→组件”的映射加载到内存哈希表**中，形成可快速查询的路由索引。  
